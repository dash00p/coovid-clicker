...
class DeprecatedCoreComponent<T extends ICoreComponentProps = ICoreComponentProps> extends HTMLElement {
  ...
  protected constructor(props: T) {
    super();
    if (new.target === DeprecatedCoreComponent) {
      throw new TypeError("Cannot construct CoreComponent instances directly");
    }
  ...
  listen(
    ref: DeprecatedCoreComponent<T>,
    name: string,
    initialValue: string | number | object,
    callbackList: Function[]
  ): void {
    const parent: DeprecatedCoreComponent<T> = ref;
  ...
  static createElementOld(type: string, options?: ICreateElementOptions | string) {
    const el: HTMLElement = document.createElement(type);
  ...
  removeAllChild(parentNode?: ParentNode): void {
    ...
    while (el.lastChild) {
      if (typeof (el.lastChild as DeprecatedCoreComponent<T>).clearActiveJobs === "function") {
        (el.lastChild as DeprecatedCoreComponent<T>).clearActiveJobs();
      }
...
export default DeprecatedCoreComponent;
